...
class MSCCustomGeometryView: SCNView
{
  var model: MSCCustomGeometryModel = MSCCustomGeometryModel()
  ...
  {
    scene?.rootNode.cleanup()
    self.model.configure(with: project)
    setupScene()
  }
  ...
  private func drawReceivers()
  {
    if (self.model.profileArray.count != 0) {
      ...
      let globalPositions : [SCNVector3] = self.model.profileArray as! [SCNVector3]
  ...
  private func drawModel()
  {
    ...
    {
      selectedNumberMaterials.append(Int(material.numberOfMaterial))
    }
    ...
    for hexahedron in self.model.elementsArray
    {
      ...
      if !selectedNumberMaterials.contains(Int(hexahedron.material))
      {
        continue
      }
      var indices: [CInt] = []
      ...
      var hexaPositions: [SCNVector3] = []
      for posValue in hexahedron.positions {
        let vecValue: SCNVector3 = posValue as! SCNVector3
        hexaPositions.append(vecValue)
      }
      
      globalPositions = globalPositions + hexaPositions
  ...
  private func drawReceiversSurface()
  {
    ...
    {
      let indicesSide = triangle.indicesArray(withAddValue: h * 3)
      var indicesSideArray: [CInt] = []
      for indice in indicesSide! {
        indicesSideArray.append(CInt(indice.intValue))
      }
      let indexDataSide = Data(bytes: indicesSide!,
                               count: MemoryLayout<CInt>.size * (indicesSide?.count)!)
      let elementSide = SCNGeometryElement(data: indexDataSide,
                                           ...
                                           bytesPerIndex: MemoryLayout<CInt>.size)
      globalElements.append(elementSide)
      indices = indices + indicesSideArray
      
      var triangleArray: [SCNVector3] = []
      for trianglePos in triangle.positions {
        let vectorTriangle = trianglePos as! SCNVector3
        triangleArray.append(vectorTriangle)
      }
      vertexPositions += triangleArray
      
      var triangleColors: [SCNVector3] = []
      for triangleColor in triangle.colors {
        let triangleColorValue = triangleColor as! SCNVector3
        triangleColors.append(triangleColorValue)
      }
  
      globalColors = globalColors + triangleColors
      h += 1
    }
